...
export const missoes = [
  {
    ...
    resumo: { pt: 'o nome e o grupo de renda de cada país, com títulos que qualquer pessoa entenda.', en: 'each country’s name and income group, with headings anyone understands.' },
    conceitosNovos: ['AS'],
    tabelas: ['countries'],
    conceito: {
      pt: '`AS` dá um apelido a uma coluna do resultado: o dado continua o mesmo, só muda o nome no cabeçalho. No exemplo ao lado, `AS indicador` troca o título da coluna `nome_indicador`. Um apelido com espaço vai entre aspas duplas: `AS "nome do indicador"`.',
      en: '`AS` gives a result column a nickname: the data stays the same, only the header changes. In the example alongside, `AS indicator` changes the heading of the `indicator_name` column. A nickname with spaces goes in double quotes: `AS "indicator name"`.',
    },
    ...
    passoAPasso: {
      ...
      SELECT: {
        pt: 'Depois, fica com `nome_indicador` e `unidade` — e o `AS` troca o nome delas no cabeçalho.',
        en: 'Then it keeps `indicator_name` and `unit` — and `AS` changes their names in the header.',
      },
    },
    palpite: {
...

  {
    ...
    // Uma linha que lembra o pedido na etapa do conceito.
    resumo: { pt: 'os grupos de renda, cada um uma vez só.', en: 'the income groups, each only once.' },
    conceitosNovos: ['DISTINCT'],
    tabelas: ['countries'],
    amostra: 'SELECT country_name, income_group FROM countries LIMIT 5',
    conceito: {
      pt: '`DISTINCT`, logo depois do `SELECT`, tira as linhas repetidas do resultado: cada combinação de valores aparece uma vez só. No exemplo ao lado, `SELECT DISTINCT tema` mostra cada tema uma vez, por mais indicadores que ele tenha. O vazio (`NULL`) também conta como um valor.',
      en: '`DISTINCT`, right after `SELECT`, removes repeated rows from the result: each combination of values shows up only once. In the example alongside, `SELECT DISTINCT topic` shows each topic once, however many indicators it has. The empty value (`NULL`) counts as a value too.',
    },
    ...
    passoAPasso: {
      ...
      SELECT: {
        pt: 'Depois, fica só com a coluna `tema`, e o `DISTINCT` tira os temas repetidos.',
        en: 'Then it keeps only the `topic` column, and `DISTINCT` removes the repeated topics.',
      },
    },
    palpite: {
...

  {
    ...
    // Uma linha que lembra o pedido na etapa do conceito.
    resumo: { pt: 'o nome e o salário por mês de cada pessoa.', en: 'each person’s name and monthly salary.' },
    conceitosNovos: ['CONTAS'],
    tabelas: ['staff'],
    conceito: {
      pt: 'Colunas de números aceitam contas: `+` soma, `-` subtrai, `*` multiplica e `/` divide. A conta é feita linha por linha, e o resultado vira uma coluna nova — com `AS`, ela ganha um nome legível. No exemplo ao lado, `orcamento_usd / 1000000` mostra o orçamento em milhões.',
      en: 'Number columns accept arithmetic: `+` adds, `-` subtracts, `*` multiplies and `/` divides. The calculation runs row by row, and the result becomes a new column — with `AS`, it gets a readable name. In the example alongside, `budget_usd / 1000000` shows the budget in millions.',
    },
    exemplo: {
    ...
    passoAPasso: {
      ...
      SELECT: {
        pt: 'Depois, fica com `titulo` e faz a conta em `orcamento_usd`, linha por linha.',
        en: 'Then it keeps `title` and does the maths on `budget_usd`, row by row.',
      },
    },
    palpite: {
...

  {
    ...
    // Uma linha que lembra o pedido na etapa do conceito.
    resumo: { pt: 'uma coluna só com o nome e o cargo, para os crachás.', en: 'a single column with name and job title, for the badges.' },
    conceitosNovos: ['||'],
    tabelas: ['staff'],
    conceito: {
      pt: '`||` junta textos numa coluna só. Pedaços fixos vão entre aspas simples, e tudo o que está entre as aspas aparece igualzinho, espaços inclusive. No exemplo ao lado, `||` junta `codigo_regiao`, o pedaço `\' - \'` e `nome_regiao`, e devolve linhas como "EAS - East Asia & Pacific".',
      en: '`||` joins text into a single column. Fixed pieces go in single quotes, and everything inside the quotes shows up exactly as written, spaces included. In the example alongside, `||` joins `region_code`, the piece `\' - \'` and `region_name`, returning rows like "EAS - East Asia & Pacific".',
    },
    ...
    passoAPasso: {
      FROM: {
        pt: 'Primeiro, o banco pega a tabela `regioes` inteira.',
        en: 'First, the database takes the whole `regions` table.',
      },
      SELECT: {
        pt: 'Depois, junta `codigo_regiao` e `nome_regiao` numa coluna só.',
        en: 'Then it joins `region_code` and `region_name` into a single column.',
      },
    },
    palpite: {
...

  {
    ...
    // Uma linha que lembra o pedido na etapa do conceito.
    resumo: { pt: 'as publicações, com título e data, e uma linha explicando a consulta.', en: 'the publications, with title and date, and a line explaining the query.' },
    conceitosNovos: ['--'],
    tabelas: ['publications'],
    conceito: {
      pt: 'Tudo o que vem depois de dois hífens (`--`), até o fim da linha, é comentário: o DuckDB ignora. No exemplo ao lado, a primeira linha explica a consulta a quem for ler depois — inclusive você, daqui a um mês. Também serve para desligar uma linha sem apagá-la.',
      en: 'Everything after two hyphens (`--`), up to the end of the line, is a comment: DuckDB ignores it. In the example alongside, the first line explains the query to whoever reads it later — including you, a month from now. It also switches a line off without deleting it.',
    },
    ...
    passoAPasso: {
      FROM: {
        pt: 'Primeiro, o banco pega a tabela `regioes` inteira. O comentário, lá em cima, ele simplesmente pula.',
        en: 'First, the database takes the whole `regions` table. The comment on top is simply skipped.',
      },
      SELECT: {
        pt: 'Depois, o `*` pede todas as colunas.',
        en: 'Then `*` asks for every column.',
      },
    },
    palpite: {
...

  {
    ...
    // Uma linha que lembra o pedido na etapa do conceito.
    resumo: { pt: 'só as 10 primeiras linhas de `pais_ano`.', en: 'just the first 10 rows of `country_year`.' },
    conceitosNovos: ['LIMIT'],
    tabelas: ['country_year'],
    conceito: {
      pt: '`LIMIT`, no fim da consulta, diz quantas linhas mostrar, no máximo. Sem uma ordem definida — isso vem mais adiante —, são as primeiras que o banco encontrar. É o jeito de espiar uma tabela grande sem trazer tudo: no exemplo ao lado, `LIMIT 5` mostra só 5 países.',
      en: '`LIMIT`, at the end of the query, says how many rows to show at most. Without a defined order — that comes later — they are the first ones the database finds. It is how you peek at a big table without bringing everything: in the example alongside, `LIMIT 5` shows just 5 countries.',
    },
    ...
    passoAPasso: {
      ...
      LIMIT: {
        pt: 'Por último, o `LIMIT` deixa só as 5 primeiras linhas.',
        en: 'Last, `LIMIT` keeps only the first 5 rows.',
      },
    },
    palpite: {
